...
public class ROS {
		...
		void start() {
			...
			try {
				...
				topicDefinition = new TopicDefinition(topic_name,
						MessageDefinition.createFromMessage((Message) clazz
								.newInstance()));

				publisher = new org.ros.internal.topic.Publisher(
						topicDefinition, ros.getHostName(), 0);
				publisher.start();
			} catch (IOException e) {
				e.printStackTrace();
	...
	public static interface Namespace {
		 ...
		 * @param topic_name
		 *            the topic name, will be pushed down under this namespace
		 *            unless '/' is prepended
		 * @param clazz
		 *            the Class object used to deal with type eraser
		 * @return A handle to a publisher that may be used to publish messages
		 *         of type MessageT
		 */
		public <MessageT extends Message> Publisher<MessageT> createPublisher(
				String topic_name, Class<MessageT> clazz);
		 ...
		 * @param clazz
		 *            the Class object used to deal with type eraser
		 * @return A handle to a Subscriber that may be used to subscribe
		 *         messages of type MessageT
		 */
		public <MessageT extends Message> Subscriber<MessageT> createSubscriber(
				String topic_name, Subscriber.Callback<MessageT> callback,
				Class<MessageT> clazz);
	...
	public static class Node implements Namespace {
		...
		/**
		 * This starts up a connection with the master and gets the juices
		 * flowing
		 */
		public void init() {
		 ...
		 */
		void spin() {
			// TODO spin indefinitely
		}

		@Override
		public <MessageT extends Message> Publisher<MessageT> createPublisher(
				String topic_name, Class<MessageT> clazz) {
			Publisher<MessageT> pub = new Publisher<MessageT>(
					resolveName(topic_name), clazz);
			pub.start();
			try {
				try {
		...
		@Override
		public <MessageT extends Message> Subscriber<MessageT> createSubscriber(
				String topic_name, Callback<MessageT> callback,
				Class<MessageT> clazz) {
