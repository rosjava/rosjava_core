...
public class ListField<T> extends Field {
  ...
  static <T> ListField<T> newConstant(FieldType type, String name, List<T> value) {
    return new ListField<T>(type, name, value, true);
  }

  static <T> ListField<T> newValue(FieldType type, String name) {
    return new ListField<T>(type, name, new ArrayList<T>(), false);
  }

  private ListField(FieldType type, String name, List<T> value, boolean isConstant) {
  ...
  public int getSerializedSize() {
    ...
    if (type instanceof MessageFieldType) {
      for (RawMessage rawMessage : (List<RawMessage>) value) {
        size += rawMessage.getSerializedSize();
      }
